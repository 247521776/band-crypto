...
class CosmosBuilder extends Builder {
  ...
  buildTx(tx) {
    ...
    let msg;
    switch (req.type) {
      case Config.band.tx.transfer.type: {
        msg = Bank.create(req);
        break;
      }
      ...
      case Config.band.tx.undelegate.type: {
        msg = Stake.createMsgUndelegate(req);
        break;
      }
      case Config.band.tx.beginRedelegate.type: {
        msg = Stake.createMsgBeginRedelegate(req);
        break;
      }
      case Config.band.tx.setWithdrawAddress.type: {
        msg = Distribution.CreateMsgSetWithdrawAddress(req);
        break;
      }
      case Config.band.tx.withdrawDelegatorReward.type: {
        msg = Distribution.CreateMsgWithdrawDelegatorReward(req);
        break;
      }
      case Config.band.tx.withdrawValidatorCommission.type: {
        msg = Distribution.CreateMsgWithdrawValidatorCommission(req);
        break;
      }
      case Config.band.tx.deposit.type: {
        msg = Gov.createMsgDeposit(req);
        break;
      }
      case Config.band.tx.vote.type: {
