...
class CosmosCrypto extends Crypto {
  ...
  generateMnemonic(language, mnemonicLength = 24) {
    return CosmosKeypair.generateMnemonic(
      switchToWordList(language),
      mnemonicLength
    );
  }
  ...
  recover(secret, language, path) {
    path = path || Config.band.bip39Path;
    let keyPair = CosmosKeypair.recover(
      secret,
      switchToWordList(language),
      path
    );
  ...
  getAddress(publicKey) {
    if (Codec.Bech32.isBech32(Config.band.bech32.accPub, publicKey)) {
    ...
    address = Codec.Bech32.toBech32(Config.band.bech32.accAddr, address);
    return address;
  }

  encodePublicKey(publicKey) {
    let pubkey = publicKey;
    if (Codec.Bech32.isBech32(Config.band.bech32.accPub, pubkey)) {
      pubkey = Codec.Bech32.toBech32(
        Config.band.bech32.accPub,
        Codec.Bech32.fromBech32(pubkey)
      );
    } else if (Codec.Hex.isHex(pubkey)) {
      pubkey = Codec.Bech32.toBech32(Config.band.bech32.accPub, pubkey);
    }
    return pubkey;
  }
}
...
function encode(acc) {
  if (!Utils.isEmpty(acc)) {
    switch (Config.band.defaultCoding) {
      case Config.band.coding.bech32: {
        if (Codec.Hex.isHex(acc.address)) {
          acc.address = Codec.Bech32.toBech32(
            Config.band.bech32.accAddr,
            acc.address
          );
        }
        if (Codec.Hex.isHex(acc.publicKey)) {
          acc.publicKey = Codec.Bech32.toBech32(
            Config.band.bech32.accPub,
            acc.publicKey
          );
        }
