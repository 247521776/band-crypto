...
class CosmosKeypair {
  static getPrivateKeyFromSecret(mnemonicS, path) {
    ...
    let derivedPriv = Hd.DerivePrivateKeyForPath(
      master.secret,
      master.chainCode,
      path
    );
  ...
  static sign(private_key, msg) {
    ...
    //对数据签名
    let prikeyArr = Buffer.from(
      new Uint8Array(Codec.Hex.hexToBytes(private_key))
    );
  ...
  static create(language, mnemonicLength) {
    ...
    //生成私钥
    let secretKey = this.getPrivateKeyFromSecret(
      mnemonicS,
      Config.band.bip39Path
    );
    //构造公钥
    let pubKey = Secp256k1.publicKeyCreate(secretKey);
    pubKey = Amino.MarshalBinary(Config.band.amino.pubKey, pubKey);
  ...
  static isValidAddress(address) {
    let prefix = Config.band.bech32.accAddr;
