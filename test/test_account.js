...
describe("account", function () {
  describe("iris", function () {
    ...
    it("test create and recover", function () {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        let keyPair2 = crypto.recover(
          keyPair.phrase,
          Irisnet.config.language.en
        );
    ...
    it("test generate Mnemonic", function () {
      ...
      [12, 15, 18, 21, 24].forEach((item) => {
        let mnemonics = crypto.generateMnemonic(
          Irisnet.config.language.en,
          item
        );
    ...
    it("should recover", function () {
      let crypto = Irisnet.getCrypto(chainName, "testnet");
      let seed =
        "error nerve credit mail coyote melt property afford design wool dune sibling loan tunnel acid joke father bid home pupil giant share age warrior";
    ...
    describe("keystore", function () {
      it("exportKeystore", function () {
        let privateKey =
          "55A3160577979EC014A2CE85C430E1FF0FF06EFD230B7CE41AEAE2EF00EDF175";
        let crypto = Irisnet.getCrypto(chainName, "testnet");
  ...
  describe("cosmos", function () {
    ...
    it("should utils", function () {
      ...
      assert.equal(
        utils.toDecString("10000000000000000000000"),
        "10000000000000000000000.0000000000"
      );
    });
  });

  describe("band", function () {
    let chainName = Irisnet.config.chain.band;
    it("test create and recover", function () {
      let str = "";
      for (let i = 0; i < 100; i++) {
        let crypto = Irisnet.getCrypto(chainName);
    ...
    it("test generate Mnemonic", function () {
      ...
      [12, 15, 18, 21, 24].forEach((item) => {
        ...
        console.log(
          "address:",
          crypto.recover(mnemonics, Irisnet.config.language.en).address
        );
        console.log(
          "private key:",
          crypto.recover(mnemonics, Irisnet.config.language.en).privateKey
        );
      });
    });
